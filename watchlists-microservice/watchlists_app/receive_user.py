...
sys.path.append("")
os.environ.setdefault("DJANGO_SETTINGS_MODULE", "watchlists_project.settings")
django.setup()
...
def save_user(ch, method, properties, body):
    ...
    elif data["old_username"]:
        UserVO.objects.filter(username=data["old_username"]).update(
            username=username, email=email
        )
        print("Successfully updated UserVO:", username)
    else:
